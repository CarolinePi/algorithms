...
class LinkedList:
    ...
    def insert_to_end(self, data):
        ...
        if self.begin is None:
            self.begin = self.end = Node(data, None)
        else:
            self.end.next = self.end = Node(data, None)
    ...
    def remove_to_end(self):
        if self._length == 0:
            ...
            for _ in range(self._length - 2):
                current = current.next
            current.next = Node(current.next.data, None)
    ...
    def replace_to_begin(self, data):
        self.begin = Node(data, self.begin.next)
...
s.replace_to_end(9)
s.replace_to_end(3)
s.insert_to_center(2, 2)
s.insert_to_center(3, 3)
s.insert_to_center(4, 4)
s[1]
s[1] = 16
s.get_lenght()
print(s)
